...
# -*- coding: utf-8 -*-
"""
Created on Mon Aug  9 18:39:40 2021
...
def get_mutated_mols(smi, num_random_samples, num_mutation_ls, fp_type): 
...
def mutate_selfie_substructure(selfie, max_molecules_len, write_fail_cases=False):
    '''Return a mutated selfie string (only one mutation on slefie is performed)
...
def get_mutated_SELFIES_subsrtuct(selfies_ls, num_mutations): 
    ''' Mutate all the SELFIES in 'selfies_ls' 'num_mutations' number of times. 
    ...
    for _ in range(num_mutations): 
        ...
        for str_ in selfies_ls: 
            ...
            selfie_mutated, _ = mutate_selfie_substructure(str_, max_molecules_len)
            selfie_ls_mut_ls.append(selfie_mutated)
...
def get_mutated_mols_substruct(smi, num_random_samples, num_mutation_ls): 
    ...
    for num_mutations in num_mutation_ls: 
        # Mutate the SELFIES: 
        selfies_mut = get_mutated_SELFIES_subsrtuct(selfies_ls.copy(), num_mutations=num_mutations)
